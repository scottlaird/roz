...
func TestDefaultPrefixes(t *testing.T) {
	...
	if got := addProject(t, db, "the first one"); got != "ROZ1" {
		t.Errorf("project add printed %q, want ROZ1", got)
	}
...
func TestPrefixesAreNotHardcoded(t *testing.T) {
	...
	_, err := runCLI(t, "note", "--db", path, "ROZ9", "nope")
	if err == nil {
...
func TestInitIsSafeToRerun(t *testing.T) {
	...
	if got := addProject(t, db, "the second one"); got != "ROZ2" {
		t.Errorf("project add printed %q after a re-init, want ROZ2 (was %s)", got, id)
	}
}
...
func TestInitMessage(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			want:   "initialised /tmp/roz.db (schema 15, action=NA, project=ROZ)",
		},
		{
			name:   "migrated",
			result: store.InitResult{From: 13, To: 15, Prefixes: prefixes},
			want:   "migrated /tmp/roz.db: schema 13 → 15 (action=NA, project=ROZ)",
		},
		{
			name:   "unchanged",
			result: store.InitResult{From: 15, To: 15, Prefixes: prefixes},
			want:   "/tmp/roz.db is up to date (schema 15, action=NA, project=ROZ)",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			if got := initMessage("/tmp/roz.db", tc.result); got != tc.want {
